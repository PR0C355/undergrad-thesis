...
Secondly, timestamp merging consolidates multiple timestamp entries, merging adjacent segments separated by less than 2 seconds into a single continuouos range to avoid fragmentation. Third, a timestamp jitter is applied to increase ground-truth variability. Because captions are generated from fixed $10$-second clips, LLM-generated timestamps are ordinarily constrained to $10$-second boundaries. To produce more varied annotations, the recorded start timestamp is shifter earlier by $X \sim \operatorname{Unif}(\{1, 2, 3\})$ seconds and the end timestamp later by $X \sim \operatorname{Unif}(\{1, 2, 3\})$ seconds, stopping at the existing video's boundaries (Can't be less than 0, or longer than the video). This also provide tolerance for imprecise annotations when ground-truth ranges are smaller than the clip length. 
...
ChronoQA evaluates five models spanning the full range of capabilities in the current VLM landscape, including both closed-source and open-source models.
...
\begin{table*}
  ...
  \begin{tabular}{llll}
    ...
    Qwen3-VL-8B &  Open-source & Dynamic resolution & 256K (expandable to 1M) \\
